...
private fun buildWorkspacePrompt(workspace: WorkspaceEntity, cwd: String? = null): String = buildString {
    ...
    appendLine("  - `workspace_list_files`: list directory contents.")
    appendLine("  - `workspace_search_files`: search file contents with grep.")
    appendLine("  - `workspace_shell`: run shell commands (the files area is mounted at /workspace).")
    appendLine("  - `git_clone` / `git_status` / `git_diff` / `git_commit` / `git_log`: Git operations.")
    appendLine("  - `pkg_install`: install packages (apt/pip/npm auto-detected).")
    appendLine("  - `build_project`: build project with auto-detected build system.")
    appendLine("  - `run_tests`: run tests with auto-detected test framework.")
    appendLine("  - `terminal_session_create` / `terminal_session_exec` / `terminal_session_signal`: persistent terminal sessions.")
    appendLine("- The shell now uses a hybrid execution engine: most commands run natively (fast), only apt/glibc tools use proot.")
